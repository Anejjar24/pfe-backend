...
exports.DataAggregateHandler = void 0;
class DataAggregateHandler {
    ...
    stats(input) {
        ...
        if (values.length === 0)
            return { error: 'No numeric values found in input', branch: 'error' };
    ...
    trend(node, input) {
        const values = this.numericArray(input);
        if (values.length < 2)
            return { error: 'At least 2 values required for trend detection', branch: 'error' };
        const winSize = Math.min(Number(node.data?.windowSize ?? 10), values.length);
        const window = [...values.slice(0, winSize)].reverse();
    ...
    movingAverage(node, input) {
        const values = this.numericArray(input);
        if (values.length === 0)
            return { error: 'No numeric values in input', branch: 'error' };
        const winSize = Math.min(Number(node.data?.windowSize ?? 10), values.length);
    ...
    numericArray(input) {
        ...
        if (typeof input === 'object' && input !== null) {
            ...
            if (Array.isArray(obj['sensors'])) {
                return obj['sensors'].map(s => Number(s.value)).filter(v => !isNaN(v));
            }
            if (Array.isArray(obj['items'])) {
                return obj['items'].map(Number).filter(v => !isNaN(v));
            }
        }
        return typeof input === 'number' ? [input] : [];
    }
}
exports.DataAggregateHandler = DataAggregateHandler;
//# sourceMappingURL=data-aggregate.handler.js.map