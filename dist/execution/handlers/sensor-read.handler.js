...
class SensorReadHandler {
    ...
    async execute(node, _input) {
        ...
        switch (operation) {
            ...
            default: return this.executeSingle(node);
        }
    }
    async executeSingle(node) {
    ...
    async executeHistory(node) {
        const sensorId = String(node.data?.sensorId || '').trim();
        if (!sensorId)
            return { error: 'sensorId not configured', readings: [], branch: 'error' };
    ...
    async executeBatch(node) {
        ...
        if (stationId)
            qb.where('station.id = :stationId', { stationId });
        if (sensorType !== 'all')
